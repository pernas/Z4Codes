...
two and delta rows of order four. It also returns the parameter gamma and delta. 
	...
	require (BaseRing(C) eq Z4) : "C is not a code over Z4";
...
in C \}, where K2^m is a quaternary matrix of length 2^m with the vector
 (1,3,3,1,3,1,1,3, ...) in the main diagonal and zeros elsewhere. 
    ...
    require (Length(C)) eq 2^Ilog(2,Length(C)) : 
        "C must be of length a power of two";
...
/*****************************************************************************/
/******************************************************************************
** CosetLeaders function
******************************************************************************/

intrinsic CosetLeaders(C::CodeLinRng) -> SeqEnum
{
	Given a code C over Z4 of length n, with ambient space V = Z_4^n, return 
a set of coset leaders (not necessarily of minimal weight in their cosets) 
for C in V as an indexed set of vectors from V. The set of cosett leaders 
...
intrinsic CosetLeaders(C::CodeLinRng, S::CodeLinRng) -> SeqEnum, SeqEnum
{
	Given a code C over Z4 of length n, and a subcode S over Z4 of C, 
return a set of coset leaders (not necessarily of minimal weight in their 
cosets) for S in C as an indexed set of codewords from C. The set of cosett 
leaders \{c_0, c_1,..., c_t \} satisfies that c_0 is the zero codeword, and 
C = U_(i=0)^t (S + c_i). Note that this function is only applicable when S and 
C are small.
}	
	require (BaseRing(C) eq Z4) : "C is not a code over Z4";
    require (BaseRing(S) eq Z4) : "S is not a code over Z4";
    require (S subset C): "S is not a subcode of C";

	length := Length(C);
    f2 := GrayMap(C);
    V2 := VectorSpace(GF(2),2*length);
...
/*****************************************************************************/
/******************************************************************************
** KernelCosetLeaders function
******************************************************************************/

intrinsic KernelCosetLeaders(C::CodeLinRng) -> SeqEnum, SeqEnum
{
Given a code C over Z4 of length n, return the coset leaders 
[c_1,..., c_t] as a sequence of codewords of C, such that 
C = KC U (U_(i=1)^t (KC + c_i)), where KC is the kernel of C as a 
subcode over Z4. It also returns the coset leaders of the corresponding 
...
/*****************************************************************************/
/******************************************************************************
** CardinalPAutHadamardCodeZ4 function
******************************************************************************/

intrinsic CardinalPAutHadamardCodeZ4(delta::RngIntElt, m::RngIntElt) -> RngIntElt
{
	Given an integer m >= 1 and an integer delta such that 1 <= delta <= 
...
which preserve the code.
	...
	if m eq 2*s+1 then
		return CardinalPAutHadamardCodeZ4(delta-1,m-2)*4^(s-1)*(2^(2*s+2)-2^(s+2));
	end if;
	if m gt 2*s+1 then
		return CardinalPAutHadamardCodeZ4(delta,m-1)*2^(m-s-2)*(2^(m-s)-2^(s+1));
	end if;
end intrinsic;

/*****************************************************************************/
/******************************************************************************
** CardinalPermutationGroupHadamardCodeZ4 function
******************************************************************************/

intrinsic CardinalPermutationGroupHadamardCodeZ4(delta::RngIntElt, m::RngIntElt) 
                                                                    -> RngIntElt
{
	Given an integer m >= 1 and an integer delta such that 1 <= delta <= 
floor((m + 1)/2), return the cardinal of the permutation group G of a 
...
which preserve the code.
	...
	return CardinalPAutHadamardCodeZ4(delta,m);
end intrinsic;
...
/*****************************************************************************/
/******************************************************************************
** CardinalPAutExtendedPerfectCodeZ4 function
******************************************************************************/

intrinsic CardinalPAutExtendedPerfectCodeZ4(delta::RngIntElt, m::RngIntElt) 
                                                               -> RngIntElt
{
	Given an integer m >= 2 and an integer delta such that 1 <= delta <= 
...
/*****************************************************************************/
/******************************************************************************
** CardinalPermutationGroupExtendedPerfectCodeZ4 function
******************************************************************************/

intrinsic CardinalPermutationGroupExtendedPerfectCodeZ4(delta::RngIntElt, 
...
/*****************************************************************************/
/******************************************************************************
** CardinalMAutHadamardCodeZ4 function
******************************************************************************/
